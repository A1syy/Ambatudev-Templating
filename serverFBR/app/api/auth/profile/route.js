...
export async function GET(request) {
  try {
    ...
    if (!resp.data) {
      return new Response(JSON.stringify({ error: 'Not Found', message: 'User not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    }
    const { password, ...userWithoutPassword } = resp.data;
    return new Response(JSON.stringify({ id: userId, ...userWithoutPassword }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
...
export async function PUT(request) {
  try {
    ...
    if (email && email !== updateData.email) {
      ...
      if (emailExists) return new Response(JSON.stringify({ error: 'Update Failed', message: 'Email already in use' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    ...
    if (currentPassword && newPassword) {
      ...
      if (!isValid) return new Response(JSON.stringify({ error: 'Update Failed', message: 'Current password is incorrect' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
    ...
    return new Response(JSON.stringify({ message: 'Profile updated successfully', user: { id: userId, ...userWithoutPassword } }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
    if (err instanceof Response) return err;
    return new Response(JSON.stringify({ error: 'Internal Server Error', message: err.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
