...
export async function POST(request) {
  try {
    ...
    if (existing) {
      return new Response(JSON.stringify({ error: 'Registration Failed', message: 'Email already registered' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    return new Response(JSON.stringify({ message: 'User registered successfully', token, user: { id, email, name } }), { status: 201, headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
